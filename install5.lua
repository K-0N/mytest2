...
local function tr(text)
	local s, r = pcall(function()
		return translator:Translate(player, text)
	end)

	return s and r or text
end
...
local function serverhop()
	local function serverhop()
		...
		if #servers > 0 then
			...
			return warn(tr("Couldn't find a server."))
		end
	end
...
UIGradient.Color = ColorSequence.new{
	ColorSequenceKeypoint.new(0.00, Color3.fromRGB(47, 24, 125)),
	ColorSequenceKeypoint.new(1.00, Color3.fromRGB(23, 62, 76))
}
...
UIAspectRatioConstraint_3.AspectRatio = 4
...
TextLabel_2.Text =
	tr("Wait while atlas installs the required files.") ..
	" <font color=\"rgb(100,100,100)\">" ..
	tr("Installation times depend on the speed of your internet.") ..
	"</font>"
...
label.Text =
	tr("You will only see this once.") ..
	" " ..
	tr("Note: You will rejoin the server while in the installation process.") ..
	" " ..
	tr("You will not be teleported to a different server.")
...
lc = game:GetService("RunService").Heartbeat:Connect(function(dt)
	...
	if elapsed - lastUpdate < 0.5 then
		return
	end
	...
	label.Text =
		...
		display ..
		"s\n" ..
		tr("Note: You will rejoin the server again while in the installation process.") ..
		" " ..
		tr("You will not be teleported to a different server.")
end)
...
dot3.BackgroundTransparency = 0.8
...
dot2.BackgroundTransparency = 0.8
...
local function TQOM_fake_script()
	...
	local function tbg(p, n)
		game:GetService("TweenService"):Create(
			p,
			TweenInfo.new(i * 3, Enum.EasingStyle.Linear),
